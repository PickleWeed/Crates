...
class _NearbyState extends State<Nearby> {
  ...
  Future<Set<Marker>> generateMarkersFeature() async {
    ...
    final locationIcon =
        await mapHandler.getBytesFromAsset('assets/location_icon1.png', 200);
    for (int i = 0; i < _listing.length; i++) {
      print(user);
      final marker = Marker(
          markerId: MarkerId(
              LatLng(_listing[i].latitude, _listing[i].longitude).toString()),
          position: LatLng(_listing[i].latitude, _listing[i].longitude),
          icon: BitmapDescriptor.fromBytes(markerIcon),
          onTap: () {
            markerDistance = dataHandler.haversine(_listing[i].latitude,
                _listing[i].longitude, _center.latitude, _center.longitude);
            var convertedDateTime =
                "${_listing[i].postDateTime.day.toString().padLeft(2, '0')}-${_listing[i].postDateTime.month.toString().padLeft(2, '0')}-${_listing[i].postDateTime.year.toString()}";
            setState(() {
    ...
    markers.add(Marker(
        ...
        infoWindow: InfoWindow(title: locationInfo)));
  ...
  void _runSystem() async {
    ...
    if (_filterMode == false) {
      ...
      _listing = await dataHandler.retrieveFilteredListing(
          distance, category, _center);
      ...
      _listing = await dataHandler.retrieveFilteredListing(
          _mapFilter.distance, _mapFilter.category, _center);
      _username = await dataHandler.getUsernameList(_listing);
      if (_listing.isNotEmpty) {
        _positions = mapHandler.getPositionFromListing(_listing);
        _markers = await generateMarkersFeature();
                  ...
                  children: [
                    Padding(
                      ...
                      child: IconButton(
                        ...
                        onPressed: () async {
                          _mapFilter = await Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => NearbyFilter()));
                          if (_mapFilter != null) {
                            setState(() {
                              _filterMode = true;
                              _runSystem();
